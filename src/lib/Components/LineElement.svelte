...
{:then stopInfo}
	<StopNumber orden={stopInfo.orden} />
	<div class="pb-4 ml-7">
		<div class="space-y-2">
			<div class="flex justify-between items-baseline">
				<div
					style="color: hsl({stopInfo.orden * 30} 80% 20%);"
					class="font-semibold text-2xl -mb-2"
				>
					{stopInfo.nombre}
				</div>
				<div style="color: hsl({stopInfo.orden * 30} 80% 40%);" class="text-right">
					<button
						on:click={() => {
							$favourites.favourites = [...$favourites.favourites, stopInfo.orden];
						}}
					>
						<Star weight="bold" />
					</button>
			...
			<DisplayHourTable paradaInfo={stopInfo} />
		</div>
	</div>
{:catch error}
